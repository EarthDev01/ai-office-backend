...
// (B-2: ห้าม hardcode คำตอบในโค้ด · B-9: ชื่อเมนูมาจาก connector ของ kind นั้น)

var answerCategories = []string{
	"greeting",       // ทักทาย/ถามว่าช่วยอะไรได้ (BQ-52)
	"identity",       // ถามว่าเป็นคนหรือบอท (BQ-53)
	"human_handoff",  // ขอคุยกับคน/support (BQ-39, 40)
	"technical",      // เรื่องเทคนิคระบบ error/database/server (BQ-54)
	"out_of_scope",   // นอกขอบเขตงานหลังบ้าน (BQ-56)
	"action_request", // ขอให้ทำรายการ/แก้ข้อมูล/เพิ่มลดเครดิต (BQ-51 · NG-1..3)
	"cross_service",  // ขอข้อมูลของเว็บอื่น (NG-4)
	"no_tool",        // ถามข้อมูลที่ไม่มีเครื่องมือรองรับ (B-3)
	"other",
}

// refusal categories นับเป็น "อัตราปฏิเสธ" ใน rollup
...
func builtinTools(conn *connector.Connector) []port.LLMTool {
...
func canAccess(conn *connector.Connector, rule string, t domain.AccessTicket) (bool, connector.PermissionRule) {
	if rule == "" {
	...
	return r.Allows(t.User.Permissions, t.User.Level), r
}

// lookupMenu คืนเมนู/วิธีทำที่ตรงคำค้น (สูงสุด 3 + 2) · ไม่มีข้อมูลของระบบ (ไม่ต้องกัน PII)
func lookupMenu(conn *connector.Connector, t domain.AccessTicket, query string, now time.Time) (map[string]any, *domain.Card) {
	type scored struct {
		i, s int
	}
	var ms []scored
...
func explainStatus(conn *connector.Connector, input map[string]any) map[string]any {
	...
	if entry == nil {
		// ไม่ระบุสถานะ = ขอทั้งตาราง (BQ-29 "สถานะนี้แปลว่าอะไร" แบบรวม)
