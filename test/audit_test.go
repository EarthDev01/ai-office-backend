...
func TestAudit_RolePermissionsDiff(t *testing.T) {
	...
	// อ่าน matrix ปัจจุบันก่อน แล้วแก้เฉพาะ viewer (ชุดสิทธิ์ default เปลี่ยนตามงาน w-17 ได้)
	cur := payload[struct {
		Matrix map[string][]string `json:"matrix"`
	}](t, do(t, r, req{method: http.MethodGet, path: "/api/ai/admin/role-permissions", console: vp.Token}))
	cur.Matrix["viewer"] = append(cur.Matrix["viewer"], "audit.view")
	body, _ := json.Marshal(map[string]any{"matrix": cur.Matrix})
